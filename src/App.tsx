    ...
    <div className="litha-backdrop min-h-screen px-3 py-8 sm:py-16">
      <main className="relative mx-auto w-full max-w-[430px]">
        <article className="litha-sheet relative rounded-b-[3px] text-paper shadow-[0_12px_28px_rgba(0,0,0,0.55),0_44px_88px_-12px_rgba(0,0,0,0.95)]">
          ...
          <div className="px-6 pt-9 pb-9 sm:px-8">
            {/* ── date + moon: the only loud things ── */}
                ...
                <span className="whitespace-nowrap">
                  {poya.isPoyaToday && poya.today ? (
                    ...
                    poya.daysUntil === 1 ? (
                      ...
                      <>
                        <span className="text-paper">{poya.next.en} Poya</span> in{' '}
                        {poya.daysUntil} days
                      </>
                    )
                  ) : (
            ...
            {/* ── sun & clock ── */}
            <section aria-label="Sun times and Sri Lanka clock">
            ...
            <footer className="mt-10 text-center">
              <p className="text-[10px] leading-relaxed text-paper/60">
                <span className="font-sinhala">
                  පෝය දින රජයේ ගැසට් පත්‍රය අනුව වෙනස් විය හැක.
                </span>{' '}
                Poya dates may shift per the official gazette. Sun &amp; Rahu times computed for{' '}
                {city.en} ({city.lat.toFixed(2)}° N, {city.lon.toFixed(2)}° E) in Sri Lanka time,
                UTC+5:30.
              </p>
            </footer>
          </div>
